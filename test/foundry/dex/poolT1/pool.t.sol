...
contract PoolT1BaseTest is DexFactoryBaseTest {
    ...
    function setUp() public virtual override {
        ...
        _setUpDexParams(DAI_USDC_WITH_LESS_THAN_ONE, 1.1 * 1e27, 1e4 * DAI_USDC_WITH_LESS_THAN_ONE.token0Wei, 1e4 * DAI_USDC_WITH_LESS_THAN_ONE.token0Wei, i_);
        _setUpDexParams(DAI_USDC_WITH_MORE_THAN_ONE, 0.9 * 1e27, 1e4 * DAI_USDC_WITH_MORE_THAN_ONE.token0Wei, 1e4 * DAI_USDC_WITH_MORE_THAN_ONE.token0Wei, i_);
        ...
        _setUpDexParams(DAI_USDC_WITH_LESS_ORACLE, 1e27, 1e4 * DAI_USDC_WITH_LESS_ORACLE.token0Wei, 1e4 * DAI_USDC_WITH_LESS_ORACLE.token0Wei, i_);
        ...
        {   
            ...
            _setUpDexParams(DAI_USDC_WITH_80_20, 1e27, 1e4 * DAI_USDC_WITH_80_20.token0Wei, 1e4 * DAI_USDC_WITH_80_20.token0Wei, ci_);
        }
        ...
        {   
            ...
            _setUpDexParams(DAI_USDC_WITH_50_5, 1e27, 1e4 * DAI_USDC_WITH_50_5.token0Wei, 1e4 * DAI_USDC_WITH_50_5.token0Wei, ci_);
        }
        ...
        {   
            ...
            _setUpDexParams(DAI_USDC_WITH_10_1, 1e27, 1e4 * DAI_USDC_WITH_10_1.token0Wei, 1e4 * DAI_USDC_WITH_10_1.token0Wei, ci_);
        }
        ...
        {   
            ...
            _setUpDexParams(DAI_USDC_WITH_LESS_THRESHOLD, 1e27, 1e4 * DAI_USDC_WITH_LESS_THRESHOLD.token0Wei, 1e4 * DAI_USDC_WITH_LESS_THRESHOLD.token0Wei, ci_);
        }
    ...
    function _deployPoolT1(
        address tokenZero,
        address tokenOne,
        string memory poolName,
        uint256 oracleSlots_
    ) internal returns(
        ...
        vm.startPrank(alice);
        bytes memory poolT1CreationCode = abi.encodeCall(poolT1DeploymentLogic.dexT1, (dex.token0, dex.token1, oracleSlots_));
    ...
    function _makeUserContract(address user, bool makeCode) internal {
        vm.etch(user, makeCode ? abi.encode(1) : new bytes(0));
    }
    ...
    function _setUpDexParams(
        ...
        _setUserAllowancesDefaultWithModeWithHighLimit(address(liquidity), address(admin), address(dex.token0), address(dex.dexCol), true);
        _setUserAllowancesDefaultWithModeWithHighLimit(address(liquidity), address(admin), address(dex.token1), address(dex.dexCol), true);

        _setUserAllowancesDefaultWithModeWithHighLimit(address(liquidity), address(admin), address(dex.token0), address(dex.dexColDebt), true);
        _setUserAllowancesDefaultWithModeWithHighLimit(address(liquidity), address(admin), address(dex.token1), address(dex.dexColDebt), true);

        _setUserAllowancesDefaultWithModeWithHighLimit(address(liquidity), address(admin), address(dex.token0), address(dex.dexDebt), true);
        _setUserAllowancesDefaultWithModeWithHighLimit(address(liquidity), address(admin), address(dex.token1), address(dex.dexDebt), true);
        ...
        DexAdminStrcuts.InitializeVariables memory initialize_ = DexAdminStrcuts.InitializeVariables({
            ...
            upperShiftThreshold: initializeParams_.upperShiftThreshold == 0 ?  5 * 1e4 : initializeParams_.upperShiftThreshold, // 1% = 1e4
            lowerShiftThreshold: initializeParams_.lowerShiftThreshold == 0 ?  5 * 1e4 : initializeParams_.lowerShiftThreshold,
        ...
        {   // Smart Col And Debt
            ...
            uint256 ethValue_ = _getSmartDebtOrColEthAmount(dex, dex.dexColDebt, token0ColAmt + token0DebtAmt, centerPrice);
    ...
    function _getSmartDebtOrColEthAmount(DexParams memory dexPool_, FluidDexT1 dex_, uint256 token0Amount_, uint256 centerPrice_) internal returns(uint256 ethValue_) {
    ...
    function _setDexUserAllowancesDefault(
        address dex,
        address admin,
        address user
    ) internal {
    ...
    function getUserSupplyShare(FluidDexT1 dex, address user) internal view returns(uint256 share) {
        uint userSupplyData = dex.readFromStorage(
    ...
    function getTotalSupplyShares(FluidDexT1 dex) internal view returns(uint256 totalShares) {
        totalShares = dex.readFromStorage(
            bytes32(uint256(2))
        );
        return totalShares;
    }
    ...
    function getUserBorrowShare(FluidDexT1 dex, address user) internal view returns(uint256 share) {
        uint userBorrowData = dex.readFromStorage(
            LiquiditySlotsLink.calculateMappingStorageSlot(
                5,
                user
            )
    ...
    function getTotalBorrowShares(FluidDexT1 dex) internal view returns(uint256 totalShares_) {
        totalShares_ = dex.readFromStorage(
            bytes32(uint256(4))
        );
        return totalShares_;
    }
    ...
    function _getDexVariablesData(FluidDexT1 dex) internal view returns(DexVariablesData memory d_) {
        ...
        d_.lastToLastPrice = (d_.lastToLastPrice >> DEFAULT_EXPONENT_SIZE) << (d_.lastToLastPrice & DEFAULT_EXPONENT_MASK);
    ...
    function _getDexVariables2Data(FluidDexT1 dex) internal view returns(DexVariables2Data memory d2_) {
        ...
        d2_.minCenterPrice = (d2_.minCenterPrice >> DEFAULT_EXPONENT_SIZE) << (d2_.minCenterPrice & DEFAULT_EXPONENT_MASK);
        d2_.maxCenterPrice = (d2_.maxCenterPrice >> DEFAULT_EXPONENT_SIZE) << (d2_.maxCenterPrice & DEFAULT_EXPONENT_MASK);
    }
    ...
    function _getDexSupplyData(FluidDexT1 dex, address user_) internal view returns(DexUserSupplyData memory us_) {
        uint userSupplyData_ = dex.readFromStorage(
            LiquiditySlotsLink.calculateMappingStorageSlot(
                3,
                user_
            )
        ...
        us_.previousUserLimit = (us_.previousUserLimit >> DEFAULT_EXPONENT_SIZE) << (us_.previousUserLimit & DEFAULT_EXPONENT_MASK);
    ...
    function _getDexBorrowData(FluidDexT1 dex, address user_) internal view returns(DexUserBorrowData memory ub_) {
        uint userBorrowData_ = dex.readFromStorage(
            LiquiditySlotsLink.calculateMappingStorageSlot(
                5,
                user_
            )
        ...
        ub_.previousUserLimit = (ub_.previousUserLimit >> DEFAULT_EXPONENT_SIZE) << (ub_.previousUserLimit & DEFAULT_EXPONENT_MASK);
    ...
    function _getOracleMapData(FluidDexT1 dex, uint256 oracleMap_, uint256 oracleSlot_) internal returns (OracleMapData memory data_, OracleSlotConfig memory slotConfig) {
        uint256 oracleMapData_ = dex.readFromStorage(
            _calculateMappingStorageSlotForUint256(6, oracleMap_)
        );
        ...
        if ((oracleMapData_ >> (shift_)) & X9 == 0 && (isVeryFirstSlot_ || ((oracleMapData_ >> (shift_ + 9)) & X22 != 0) )) {
    ...
    function _getOracleAllSlotsData(FluidDexT1 dex) internal returns (OracleMapData[] memory data_) {
        ...
        uint256 j = 8;
        for (uint i = 0; i <= oracleMap_; i++) {
            uint256 oracleMapData_ = dex.readFromStorage(
                _calculateMappingStorageSlotForUint256(6, i)
            );
    ...
    function validatePricesOfPoolAfterSwap(FluidDexT1 dex, string memory assertMessage_) internal {
        ...
        FluidDexT1.CollateralReserves memory c_ = dex.getCollateralReserves(pex_.geometricMean, pex_.upperRange, pex_.lowerRange, pex_.supplyToken0ExchangePrice, pex_.supplyToken1ExchangePrice);
        FluidDexT1.DebtReserves memory d_ = dex.getDebtReserves(pex_.geometricMean, pex_.upperRange, pex_.lowerRange, pex_.borrowToken0ExchangePrice, pex_.borrowToken1ExchangePrice);
        ...
        // console.log("collPrice_", collPrice_);
        // console.log("debtPrice_", debtPrice_);
        assertApproxEqAbs(collPrice_, debtPrice_, 1000, string(abi.encodePacked(assertMessage_, " col price and debt price is not same after swap")));
    }
    ...
    function _getDexType(DexParams memory dexPool_, DexType dexType_, bool skipValidation) internal returns (FluidDexT1 dex_) {
    ...
    function getState(DexParams memory dexPool_, FluidDexT1 dex_, address user_) internal returns(StateData memory ) {
        ...
        (FluidLiquidityResolver.UserSupplyData memory userSupplyDataToken0_, ) = resolver.getUserSupplyData(address(dex_), dexPool_.token0);
        (FluidLiquidityResolver.UserSupplyData memory userSupplyDataToken1_, ) = resolver.getUserSupplyData(address(dex_), dexPool_.token1);
        (FluidLiquidityResolver.UserBorrowData memory userBorrowDataToken0_, ) = resolver.getUserBorrowData(address(dex_), dexPool_.token0);
        (FluidLiquidityResolver.UserBorrowData memory userBorrowDataToken1_, ) = resolver.getUserBorrowData(address(dex_), dexPool_.token1);
    ...
    function _getPricesAndExchangePrices(FluidDexT1 dex_) internal returns ( DexStrcuts.PricesAndExchangePrice memory pex_) {
        (bool status_, bytes memory returnData_) = address(dex_).call(abi.encodeWithSelector(FluidDexT1.getPricesAndExchangePrices.selector));
    ...
    function _convertAdjustedToNormal(uint256 adjustedAmt_, uint256 weiAmount_) internal returns(uint256 convertedAmount_) {
        if (weiAmount_ == 1e12) {
    ...
    function _validatePerfectAmounts(
        ...
        _comparePrecision(
            ...
            _convertNormalToAdjustedToNormal(
                isSupply ? s_.liquidityToken0SupplyReserve : s_.liquidityToken0BorrowReserve,
                dexPool_.token0Wei
        ...
        _comparePrecision(
            token1Amount_,
            _convertNormalToAdjustedToNormal(
                isSupply ? s_.liquidityToken1SupplyReserve : s_.liquidityToken1BorrowReserve,
                dexPool_.token1Wei
            ) * shareAmount_ / (isSupply ? s_.totalSupplyShares : s_.totalBorrowShares),
            dexPool_.token1Wei,
            0,
    ...
    function _testDepositPerfectColLiquidity(DexParams memory dexPool_, address user_, uint256 shareAmount_, DexType dexType_) internal returns (uint256 token0Amount_, uint256 token1Amount_) {
        ...
        (token0Amount_, token1Amount_) = dex_.depositPerfect{value: ethValue_}(
                shareAmount_,
                type(uint256).max,
                type(uint256).max,
                false
        ...
        assertApproxEqAbs(postState_.liquidityToken0Balance - preState_.liquidityToken0Balance, token0Amount_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token0 is not expected")));
        assertApproxEqAbs(postState_.liquidityToken1Balance - preState_.liquidityToken1Balance, token1Amount_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token1 is not expected")));
        assertApproxEqAbs(preState_.userToken0Balance - postState_.userToken0Balance, token0Amount_, 0, string(abi.encodePacked(assertMessage_, "user balance token0 is not expected")));
        assertApproxEqAbs(preState_.userToken1Balance - postState_.userToken1Balance, token1Amount_, 0, string(abi.encodePacked(assertMessage_, "user balance token1 is not expected")));


        assertApproxEqAbs(postState_.totalSupplyShares - preState_.totalSupplyShares, shareAmount_, 0, string(abi.encodePacked(assertMessage_, "total supply shares is not expected")));
        assertApproxEqAbs(postState_.totalBorrowShares - preState_.totalBorrowShares, 0, 0, string(abi.encodePacked(assertMessage_, "total borrow shares is not expected")));
        assertApproxEqAbs(postState_.userSupplyShares - preState_.userSupplyShares, shareAmount_, 262144, string(abi.encodePacked(assertMessage_, "user supply share is not expected")));
        assertApproxEqAbs(postState_.userBorrowShares - preState_.userBorrowShares, 0, 0, string(abi.encodePacked(assertMessage_, "user borrow share is not expected")));
    ...
    function _testWithdrawPerfectColLiquidity(
        ...
        vm.prank(address(user_));
        (token0AmountWithdraw_, token1AmountWithdraw_) = dex_.withdrawPerfect(
            shareAmount_,
            0,
            0,
            false
        ...
        // Validate Liquidity and user balances of token0 and token1
        assertApproxEqAbs(preState_.liquidityToken0Balance - postState_.liquidityToken0Balance, token0AmountWithdraw_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token0 is not expected")));
        assertApproxEqAbs(preState_.liquidityToken1Balance - postState_.liquidityToken1Balance, token1AmountWithdraw_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token1 is not expected")));
        assertApproxEqAbs(postState_.userToken0Balance - preState_.userToken0Balance, token0AmountWithdraw_, 0, string(abi.encodePacked(assertMessage_, "user balance token0 is not expected")));
        assertApproxEqAbs(postState_.userToken1Balance - preState_.userToken1Balance, token1AmountWithdraw_, 0, string(abi.encodePacked(assertMessage_, "user balance token1 is not expected")));

        assertApproxEqAbs(preState_.totalSupplyShares - postState_.totalSupplyShares, shareAmount_, 0, string(abi.encodePacked(assertMessage_, "total supply shares is not expected")));
        assertApproxEqAbs(preState_.totalBorrowShares - postState_.totalBorrowShares, 0, 0, string(abi.encodePacked(assertMessage_, "total borrow shares is not expected")));
        assertApproxEqAbs(preState_.userSupplyShares - postState_.userSupplyShares, shareAmount_, 262144, string(abi.encodePacked(assertMessage_, "user supply shares is not expected")));
        assertApproxEqAbs(preState_.userBorrowShares - postState_.userBorrowShares, 0, 0, string(abi.encodePacked(assertMessage_, "user borrow shares is not expected")));
    ...
    function _testWithdrawPerfectInOne(
        ...
        if (revertReason_.length > 0) vm.expectRevert(revertReason_);
        vm.prank(address(user_));
        uint256 withdrawAmt_ = dex_.withdrawPerfectInOneToken(
            shareAmount_,
            withdrawInToken0 ? 1 : 0,
            withdrawInToken0 ? 0 : 1,
            false
        );
        if (revertReason_.length > 0) return (token0AmountWithdraw_, token1AmountWithdraw_);

        (token0AmountWithdraw_, token1AmountWithdraw_) = withdrawInToken0 ? (withdrawAmt_, uint256(0)) : (uint256(0), withdrawAmt_);
    ...
    function _testBorrowPerfectDebtLiquidity(
        ...
        if (revertReason_.length > 0) vm.expectRevert(revertReason_);
        vm.prank(address(user_));
        (token0Amount_, token1Amount_) = dex_.borrowPerfect(
            shareAmount_,
            type(uint256).min,
            type(uint256).min,
            false
        ...
        // Validate Liquidity and user balances of token0 and token1
        assertApproxEqAbs(preState_.liquidityToken0Balance - postState_.liquidityToken0Balance, token0Amount_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token0 is not expected")));
        assertApproxEqAbs(preState_.liquidityToken1Balance - postState_.liquidityToken1Balance, token1Amount_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token1 is not expected")));
        assertApproxEqAbs(postState_.userToken0Balance - preState_.userToken0Balance, token0Amount_, 0, string(abi.encodePacked(assertMessage_, "user balance token0 is not expected")));
        assertApproxEqAbs(postState_.userToken1Balance - preState_.userToken1Balance, token1Amount_, 0, string(abi.encodePacked(assertMessage_, "user balance token1 is not expected")));

        assertApproxEqAbs(postState_.totalSupplyShares - preState_.totalSupplyShares, 0, 0, string(abi.encodePacked(assertMessage_, "total supply shares is not expected")));
        assertApproxEqAbs(postState_.userSupplyShares - preState_.userSupplyShares, 0, 0, string(abi.encodePacked(assertMessage_, "user supply shares is not expected")));
        assertApproxEqAbs(postState_.totalBorrowShares - preState_.totalBorrowShares, shareAmount_, 0, string(abi.encodePacked(assertMessage_, "total borrow shares is not expected")));
        assertApproxEqAbs(postState_.userBorrowShares - preState_.userBorrowShares, shareAmount_, 262144, string(abi.encodePacked(assertMessage_, "user borrow shares is not expected")));

        validateAfterTest(dex_);
        validatePricesOfPoolAfterSwap(dex_, assertMessage_);
    }

    function _testPaybackPerfectDebtLiquidity(DexParams memory dexPool_, address user_, uint256 shareAmount_, DexType dexType_) internal returns (uint256 token0AmountPayback, uint256 token1AmountPayback) {
        _makeUserContract(user_, true);
        _makeUserContract(bob, true);
        FluidDexT1 dex_ = _getDexType(dexPool_, dexType_, false);
        ...
        vm.prank(address(user_));
        (token0AmountPayback, token1AmountPayback) = dex_.paybackPerfect{value: ethValue_}(
            shareAmount_,
            type(uint256).max,
            type(uint256).max,
            false
        ...
        // Validate Liquidity and user balances of token0 and token1
        assertApproxEqAbs(postState_.liquidityToken0Balance - preState_.liquidityToken0Balance, token0AmountPayback, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token0 is not expected")));
        assertApproxEqAbs(postState_.liquidityToken1Balance - preState_.liquidityToken1Balance, token1AmountPayback, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token1 is not expected")));
        assertApproxEqAbs(preState_.userToken0Balance - postState_.userToken0Balance, token0AmountPayback, 0, string(abi.encodePacked(assertMessage_, "user balance token0 is not expected")));
        assertApproxEqAbs(preState_.userToken1Balance - postState_.userToken1Balance, token1AmountPayback, 0, string(abi.encodePacked(assertMessage_, "user balance token1 is not expected")));

        assertApproxEqAbs(preState_.totalSupplyShares - postState_.totalSupplyShares, 0, 0, string(abi.encodePacked(assertMessage_, "total supply shares is not expected")));
        assertApproxEqAbs(preState_.totalBorrowShares - postState_.totalBorrowShares, shareAmount_, 0, string(abi.encodePacked(assertMessage_, "total borrow shares is not expected")));
        assertApproxEqAbs(preState_.userSupplyShares - postState_.userSupplyShares, 0, 0, string(abi.encodePacked(assertMessage_, "user supply shares is not expected")));
        assertApproxEqAbs(preState_.userBorrowShares - postState_.userBorrowShares, shareAmount_, 262144, string(abi.encodePacked(assertMessage_, "user borrow shares is not expected")));

        validateAfterTest(dex_);
        validatePricesOfPoolAfterSwap(dex_, assertMessage_);
    }

    function _testPaybackPerfectInOneToken(DexParams memory dexPool_, address user_, uint256 shareAmount_, bool paybackInToken0_, DexType dexType_) internal returns (uint256 token0AmountPayback, uint256 token1AmountPayback) {
        _makeUserContract(user_, true);
        _makeUserContract(bob, true);
        FluidDexT1 dex_ = _getDexType(dexPool_, dexType_, false);
        ...
        uint256 ethValue_ = 0;
        if ((dexPool_.token0 == address(NATIVE_TOKEN_ADDRESS) && paybackInToken0_) || (dexPool_.token1 == address(NATIVE_TOKEN_ADDRESS) && !paybackInToken0_)) {
            ethValue_ = user_.balance;
        }
        ...
        (token0AmountPayback, token1AmountPayback) = paybackInToken0_ ? (paybackAmt_, uint256(0)) : (uint256(0), paybackAmt_);
        ...
        assertApproxEqAbs(preState_.totalBorrowShares - postState_.totalBorrowShares, shareAmount_, 0, string(abi.encodePacked(assertMessage_, "total borrow shares is not expected")));
        assertApproxEqAbs(preState_.userSupplyShares - postState_.userSupplyShares, 0, 0, string(abi.encodePacked(assertMessage_, "user supply shares is not expected")));
        assertApproxEqAbs(preState_.userBorrowShares - postState_.userBorrowShares, shareAmount_, 0, string(abi.encodePacked(assertMessage_, "user borrow shares is not expected")));

        validateAfterTest(dex_);
        validatePricesOfPoolAfterSwap(dex_, assertMessage_);
    }

    function _getSwapEthAmount(DexParams memory dexPool_, bool swap0to1_, address user_, uint256 amountIn_) internal returns (uint256 ethValue_){
        if (
    ...
    function _testSwapExactIn(
        ...
        if (dexType_ == DexType.SmartColAndDebt) {
            ...
            if (!skipDepositOrBorrow) _testBorrowPerfectDebtLiquidity(dexPool_, bob, 1e4 * 1e18, dexType_, new bytes(0));
        } else if (dexType_ == DexType.SmartCol) {
            // Deposit liquidity for swap, 100k
            if (!skipDepositOrBorrow) _testDepositPerfectColLiquidity(dexPool_, bob, 1e4 * 1e18, dexType_);
        }

        d_.assertMessage = string(abi.encodePacked(
            "_testSwapExactIn::",
            _getDexPoolName(dexPool_, dexType_),
            ":",
...

        {
            ...
            d_.weiDiff = 1;
            if (swap0to1_) {
                assertApproxEqAbs(d_.postState.liquidityToken0Balance - d_.preState.liquidityToken0Balance, amountIn_, d_.weiDiff, string(abi.encodePacked(d_.assertMessage, "liquidity balance token0 is not expected")));
                assertApproxEqAbs(d_.preState.liquidityToken1Balance - d_.postState.liquidityToken1Balance, amountOut_, d_.weiDiff, string(abi.encodePacked(d_.assertMessage, "liquidity balance token1 is not expected")));
                assertApproxEqAbs(d_.preState.userToken0Balance - d_.postState.userToken0Balance, amountIn_, d_.weiDiff, string(abi.encodePacked(d_.assertMessage, "user balance token0 is not expected")));
                assertApproxEqAbs(d_.postState.userToken1Balance - d_.preState.userToken1Balance, amountOut_, d_.weiDiff, string(abi.encodePacked(d_.assertMessage, "user balance token1 is not expected")));
            } else {
                assertApproxEqAbs(d_.postState.liquidityToken1Balance - d_.preState.liquidityToken1Balance, amountIn_, d_.weiDiff, string(abi.encodePacked(d_.assertMessage, "liquidity balance token1 is not expected")));
                assertApproxEqAbs(d_.preState.liquidityToken0Balance - d_.postState.liquidityToken0Balance, amountOut_, d_.weiDiff, string(abi.encodePacked(d_.assertMessage, "liquidity balance token0 is not expected")));
                assertApproxEqAbs(d_.preState.userToken1Balance - d_.postState.userToken1Balance, amountIn_, d_.weiDiff, string(abi.encodePacked(d_.assertMessage, "user balance token1 is not expected")));
                assertApproxEqAbs(d_.postState.userToken0Balance - d_.preState.userToken0Balance, amountOut_, d_.weiDiff, string(abi.encodePacked(d_.assertMessage, "user balance token0 is not expected")));
            }
        }
    ...
    function _testSwapExactOut(
        ...
        uint256 weiDiff_ = 1;
        if (swap0to1_) {
            assertApproxEqAbs(postState_.liquidityToken0Balance - preState_.liquidityToken0Balance, amountIn_, weiDiff_, string(abi.encodePacked(assertMessage_, "liquidity balance token0 is not expected")));
            assertApproxEqAbs(preState_.liquidityToken1Balance - postState_.liquidityToken1Balance, amountOut_, weiDiff_, string(abi.encodePacked(assertMessage_, "liquidity balance token1 is not expected")));
            assertApproxEqAbs(preState_.userToken0Balance - postState_.userToken0Balance, amountIn_, weiDiff_, string(abi.encodePacked(assertMessage_, "user balance token0 is not expected")));
            assertApproxEqAbs(postState_.userToken1Balance - preState_.userToken1Balance, amountOut_, weiDiff_, string(abi.encodePacked(assertMessage_, "user balance token1 is not expected")));
        } else {
            assertApproxEqAbs(postState_.liquidityToken1Balance - preState_.liquidityToken1Balance, amountIn_, weiDiff_, string(abi.encodePacked(assertMessage_, "liquidity balance token1 is not expected")));
            assertApproxEqAbs(preState_.liquidityToken0Balance - postState_.liquidityToken0Balance, amountOut_, weiDiff_, string(abi.encodePacked(assertMessage_, "liquidity balance token0 is not expected")));
            assertApproxEqAbs(preState_.userToken1Balance - postState_.userToken1Balance, amountIn_, weiDiff_, string(abi.encodePacked(assertMessage_, "user balance token1 is not expected")));
            assertApproxEqAbs(postState_.userToken0Balance - preState_.userToken0Balance, amountOut_, weiDiff_, string(abi.encodePacked(assertMessage_, "user balance token0 is not expected")));
        }
    ...
    function _testDepositColLiquidity(DexParams memory dexPool_, address user_, uint256 token0Amount_, uint256 token1Amount_, DexType dexType_) internal returns (uint256 share_) {
        return _testDepositColLiquidityInWei(dexPool_, user_, token0Amount_ * dexPool_.token0Wei, token1Amount_ * dexPool_.token1Wei, dexType_);
    }

    function _testDepositColLiquidityInWei(DexParams memory dexPool_, address user_, uint256 token0Amount_, uint256 token1Amount_, DexType dexType_) internal returns (uint256 share_) {
        _makeUserContract(user_, true);
        vm.prank(address(user_));
        FluidDexT1 dex_ = _getDexType(dexPool_, dexType_, false);
        ...
        uint256 ethValue_ = 0;
        if ((dexPool_.token0 == address(NATIVE_TOKEN_ADDRESS) && token0Amount_ > 0) || (dexPool_.token1 == address(NATIVE_TOKEN_ADDRESS) && token1Amount_ > 0)) {
            ethValue_ = user_.balance;
        }
        ...
        if (preState_.liquidityToken1SupplyReserve == 0 || preState_.liquidityToken0SupplyReserve == 0) {
            vm.expectRevert(abi.encodeWithSelector(FluidDexErrors.FluidDexError.selector, FluidDexTypes.DexT1__TokenReservesTooLow));
        }
        (share_) = dex_.deposit{value: ethValue_}(
            token0Amount_,
            token1Amount_,
            type(uint256).min,
            false
        ...
        assertApproxEqAbs(postState_.totalSupplyShares - preState_.totalSupplyShares, share_, shareDelta_, string(abi.encodePacked(assertMessage_, "total supply shares is not expected")));
        assertApproxEqAbs(postState_.totalBorrowShares - preState_.totalBorrowShares, 0, 0, string(abi.encodePacked(assertMessage_, "total borrow shares is not expected")));
        assertApproxEqAbs(postState_.userSupplyShares - preState_.userSupplyShares, share_, shareDelta_, string(abi.encodePacked(assertMessage_, "user supply shares is not expected")));
        assertApproxEqAbs(postState_.userBorrowShares - preState_.userBorrowShares, 0, 0, string(abi.encodePacked(assertMessage_, "user borrow shares is not expected")));
    ...
    function _testWithdrawColLiquidity(
        ...
        // Validate Liquidity and user balances of token0 and token1
        assertApproxEqAbs(preState_.liquidityToken0Balance - postState_.liquidityToken0Balance, token0AmountInWei_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token0 is not expected")));
        assertApproxEqAbs(preState_.liquidityToken1Balance - postState_.liquidityToken1Balance, token1AmountInWei_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token1 is not expected")));
        assertApproxEqAbs(postState_.userToken0Balance - preState_.userToken0Balance, token0AmountInWei_, 0, string(abi.encodePacked(assertMessage_,  "user balance token0 is not expected")));
        assertApproxEqAbs(postState_.userToken1Balance - preState_.userToken1Balance, token1AmountInWei_, 0, string(abi.encodePacked(assertMessage_, "user balance token1 is not expected")));

        uint256 shareDelta_ = 262144;
        assertApproxEqAbs(preState_.totalSupplyShares - postState_.totalSupplyShares, share_, shareDelta_, string(abi.encodePacked(assertMessage_, "total supply shares is not expected")));
        assertApproxEqAbs(preState_.totalBorrowShares - postState_.totalBorrowShares, 0, 0, string(abi.encodePacked(assertMessage_, "total borrow shares is not expected")));
        assertApproxEqAbs(preState_.userSupplyShares - postState_.userSupplyShares, share_, shareDelta_, string(abi.encodePacked(assertMessage_, "user supply shares is not expected")));
        assertApproxEqAbs(preState_.userBorrowShares - postState_.userBorrowShares, 0, 0, string(abi.encodePacked(assertMessage_, "user borrow shares is not expected")));
    ...
    function _testBorrowDebtLiquidity(
        ...
        return _testBorrowDebtLiquidityInWei(dexPool_, user_, token0Amount_ * dexPool_.token0Wei, token1Amount_ * dexPool_.token1Wei, dexType_, revertReason_);
    }
    ...
    function _testBorrowDebtLiquidityInWei(
        ...
        uint256 shareDelta_ = 262144;
        assertApproxEqAbs(postState_.totalSupplyShares - preState_.totalSupplyShares, 0, 0, string(abi.encodePacked(assertMessage_, "total supply shares is not expected")));
        assertApproxEqAbs(postState_.totalBorrowShares - preState_.totalBorrowShares, share_, shareDelta_, string(abi.encodePacked(assertMessage_, "total borrow shares is not expected")));
        assertApproxEqAbs(postState_.userSupplyShares - preState_.userSupplyShares, 0, 0, string(abi.encodePacked(assertMessage_, "user supply shares is not expected")));
        assertApproxEqAbs(postState_.userBorrowShares - preState_.userBorrowShares, share_, shareDelta_, string(abi.encodePacked(assertMessage_, "user borrow shares is not expected")));

        validateAfterTest(dex_);
        validatePricesOfPoolAfterSwap(dex_, assertMessage_);
    }

    function _testPaybackDebtLiquidity(DexParams memory dexPool_, address user_, uint256 token0Amount_, uint256 token1Amount_, DexType dexType_) internal returns (uint256 share_) {
        _makeUserContract(user_, true);
        vm.prank(address(user_));
        FluidDexT1 dex_ = _getDexType(dexPool_, dexType_, false);
        ...
        StateData memory preState_ = getState(dexPool_, dex_, address(user_));
        vm.prank(address(user_));
        (share_) = dex_.payback{value: ethValue_}(
            token0AmountInWei_,
            token1AmountInWei_,
            type(uint256).min,
            false
        ...
        assertApproxEqAbs(postState_.liquidityToken0Balance - preState_.liquidityToken0Balance, token0AmountInWei_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token0 is not expected")));
        assertApproxEqAbs(postState_.liquidityToken1Balance - preState_.liquidityToken1Balance, token1AmountInWei_, 0, string(abi.encodePacked(assertMessage_, "liquidity balance token1 is not expected")));
        assertApproxEqAbs(preState_.userToken0Balance - postState_.userToken0Balance, token0AmountInWei_, 0, string(abi.encodePacked(assertMessage_,  "user balance token0 is not expected")));
        assertApproxEqAbs(preState_.userToken1Balance - postState_.userToken1Balance, token1AmountInWei_, 0, string(abi.encodePacked(assertMessage_, "user balance token1 is not expected")));

        uint256 shareDelta_ = 262144;
        assertApproxEqAbs(preState_.totalSupplyShares - postState_.totalSupplyShares, 0, 0, string(abi.encodePacked(assertMessage_, "total supply shares is not expected")));
        assertApproxEqAbs(preState_.totalBorrowShares - postState_.totalBorrowShares, share_, shareDelta_, string(abi.encodePacked(assertMessage_, "total borrow shares is not expected")));
        assertApproxEqAbs(preState_.userSupplyShares - postState_.userSupplyShares, 0, 0, string(abi.encodePacked(assertMessage_, "user supply shares is not expected")));
        assertApproxEqAbs(preState_.userBorrowShares - postState_.userBorrowShares, share_, shareDelta_, string(abi.encodePacked(assertMessage_, "user borrow shares is not expected")));

        validateAfterTest(dex_);
        validatePricesOfPoolAfterSwap(dex_, assertMessage_);
    }

    function _testSwapExactInBackAndForth(DexParams memory dexPool_, address user_, uint256 amountIn_, bool startSwap0To1, DexType dexType_) internal returns (uint256 share_) {
        _makeUserContract(user_, true);
        FluidDexT1 dex_ = _getDexType(dexPool_, dexType_, false);
        ...
        uint256 amountInBack_ = _testSwapExactIn(dexPool_, user_, amountOut_, !startSwap0To1, dexType_, true, false, false);

        assertGe(amountIn_ * 10001 / 10000, amountInBack_, string(abi.encodePacked(assertMessage_, "amountIn_ is less than amountInBack_")));

        // TODO: adjust percision
        if (dexType_ == DexType.SmartColAndDebt) {
            assertApproxEqRel(amountIn_, amountInBack_, 2 * 1e16, string(abi.encodePacked(assertMessage_, "amountInBack_ is very less than amountIn_")));
        } else {
            // TODO: convert to abs value
            assertApproxEqRel(amountIn_, amountInBack_, 0.0000005 * 1e18 , string(abi.encodePacked(assertMessage_, "amountInBack_ is very less than amountIn_")));
        }
    }


    // Admin Module Helpers
    function _testUpdateRangePercents(DexParams memory dexPool_, address admin_, uint upperPercent_, uint lowerPercent_, uint shiftTime_,  DexType dexType_) internal {
        FluidDexT1 dex_ = _getDexType(dexPool_, dexType_, false);
        ...
        assertEq(shiftTime_ > 0, postDs_.d2.isPercentChangeActive, string(abi.encodePacked(assertMessage_, "isPercentChangeActive not match")));
        assertEq(upperPercent_, postDs_.d2.upperPercent, string(abi.encodePacked(assertMessage_, "upperPercent not match")));
        assertEq(lowerPercent_, postDs_.d2.lowerPercent, string(abi.encodePacked(assertMessage_, "lowerPercent not match")));

        if (shiftTime_ > 0) {
            assertEq(preDs_.d2.upperPercent, postDs_.rs.oldUpperShift, string(abi.encodePacked(assertMessage_, "DexRangeShiftData: oldUpperShift doesn't match")));
            assertEq(preDs_.d2.lowerPercent, postDs_.rs.oldLowerShift, string(abi.encodePacked(assertMessage_, "DexRangeShiftData: oldLowerShift doesn't match")));
            assertEq(shiftTime_, postDs_.rs.shiftTime, string(abi.encodePacked(assertMessage_, "DexRangeShiftData: shiftTime doesn't match")));
            assertEq(block.timestamp, postDs_.rs.timestampOfShiftStart, string(abi.encodePacked(assertMessage_, "DexRangeShiftData: timestampOfShiftStart doesn't match")));
        }
 
        // Dex Variables
        {
            assertEq(preDs_.d.isEntrancy, postDs_.d.isEntrancy, string(abi.encodePacked(assertMessage_, "isEntrancy")));
            assertEq(preDs_.d.lastToLastPrice, postDs_.d.lastToLastPrice, string(abi.encodePacked(assertMessage_, "lastToLastPrice")));
            assertEq(preDs_.d.lastPrice, postDs_.d.lastPrice, string(abi.encodePacked(assertMessage_, "lastPrice")));
            assertEq(preDs_.d.centerPrice, postDs_.d.centerPrice, string(abi.encodePacked(assertMessage_, "centerPrice")));
            assertEq(preDs_.d.lastTimestamp, postDs_.d.lastTimestamp, string(abi.encodePacked(assertMessage_, "lastTimestamp")));
            assertEq(preDs_.d.lastTimestampDifBetweenLastToLastPrice, postDs_.d.lastTimestampDifBetweenLastToLastPrice, string(abi.encodePacked(assertMessage_, "lastTimestampDifBetweenLastToLastPrice")));
        ...
        // Dex Variables 2
        {
            assertEq(preDs_.d2.isSmartColEnabled, postDs_.d2.isSmartColEnabled, string(abi.encodePacked(assertMessage_, "isSmartColEnabled")));
            assertEq(preDs_.d2.isSmartDebtEnabled, postDs_.d2.isSmartDebtEnabled, string(abi.encodePacked(assertMessage_, "isSmartDebtEnabled")));
            assertEq(preDs_.d2.fee, postDs_.d2.fee, string(abi.encodePacked(assertMessage_, "fee")));
            assertEq(preDs_.d2.revenueCut, postDs_.d2.revenueCut, string(abi.encodePacked(assertMessage_, "revenueCut")));
            ...
            assertEq(preDs_.d2.minCenterPrice, postDs_.d2.minCenterPrice, string(abi.encodePacked(assertMessage_, "minCenterPrice")));
            assertEq(preDs_.d2.maxCenterPrice, postDs_.d2.maxCenterPrice, string(abi.encodePacked(assertMessage_, "maxCenterPrice")));
            assertEq(preDs_.d2.utilizationLimitToken0, postDs_.d2.utilizationLimitToken0, string(abi.encodePacked(assertMessage_, "utilizationLimitToken0")));
            assertEq(preDs_.d2.utilizationLimitToken1, postDs_.d2.utilizationLimitToken1, string(abi.encodePacked(assertMessage_, "utilizationLimitToken1")));
    ...
    function _testUpdateThresholdPercent(
        ...
        assertEq(shiftTime_ > 0, postDs_.d2.isThresholdPercentActive, string(abi.encodePacked(assertMessage_, "isThresholdPercentActive not match")));
        assertEq(upperThresholdPercent_, postDs_.d2.upperShiftPercent, string(abi.encodePacked(assertMessage_, "upperThresholdPercent_ not match")));
        assertEq(lowerThresholdPercent_, postDs_.d2.lowerShiftPercent, string(abi.encodePacked(assertMessage_, "lowerThresholdPercent_ not match")));
        assertEq(thresholdTime_, postDs_.d2.shiftTime, string(abi.encodePacked(assertMessage_, "shiftTime not match")));

        if (shiftTime_ > 0) {
            assertEq(preDs_.d2.upperShiftPercent, postDs_.ts.oldUpperShift, string(abi.encodePacked(assertMessage_, "DexThresholdShiftData: oldUpperShift")));
            assertEq(preDs_.d2.lowerShiftPercent, postDs_.ts.oldLowerShift, string(abi.encodePacked(assertMessage_, "DexThresholdShiftData: oldLowerShift")));
            assertEq(shiftTime_, postDs_.ts.shiftTime, string(abi.encodePacked(assertMessage_, "DexThresholdShiftData: shiftTime")));
            assertEq(block.timestamp, postDs_.ts.timestampOfShiftStart, string(abi.encodePacked(assertMessage_, "DexThresholdShiftData: timestampOfShiftStart")));
            assertEq(preDs_.d2.shiftTime, postDs_.ts.oldThresholdTimestamp, string(abi.encodePacked(assertMessage_, "DexThresholdShiftData: oldThresholdTimestamp")));
        }
    ...
    function _testUpdateCenterPriceAddress(
        ...
        assertEq(centerPriceAddress_, postDs_.d2.centerPriceAddress, string(abi.encodePacked(assertMessage_, "centerPriceAddress")));
        assertEq(centerPriceAddress_ > 0, postDs_.d2.isCenterPriceShiftActive, string(abi.encodePacked(assertMessage_, "isCenterPriceShiftActive")));


        if (centerPriceAddress_ > 0) {
            assertEq(block.timestamp, postDs_.cps.timestampOfShiftStart, string(abi.encodePacked(assertMessage_, "DexCenterPriceShiftData: timestampOfShiftStart")));
            assertEq(percent_, postDs_.cps.percentShift, string(abi.encodePacked(assertMessage_, "DexCenterPriceShiftData: percentShift")));
            assertEq(time_, postDs_.cps.timeToShiftPercent, string(abi.encodePacked(assertMessage_, "DexCenterPriceShiftData: timeToShiftPercent")));
        } else {
            assertEq(0, postDs_.cps.timestampOfShiftStart, string(abi.encodePacked(assertMessage_, "DexCenterPriceShiftData: timestampOfShiftStart")));
            assertEq(0, postDs_.cps.percentShift, string(abi.encodePacked(assertMessage_, "DexCenterPriceShiftData: percentShift")));
            assertEq(0, postDs_.cps.timeToShiftPercent, string(abi.encodePacked(assertMessage_, "DexCenterPriceShiftData: timeToShiftPercent")));
        }
    ...
    function _testUpdateHookAddress(
        DexParams memory dexPool_,
        address admin_,
        uint256 hookAddress_,
        DexType dexType_
    ) internal {
        FluidDexT1 dex_ = _getDexType(dexPool_, dexType_, false);

        string memory assertMessage_ = string(abi.encodePacked(
            "_testUpdateHookAddress::",
            _getDexPoolName(dexPool_, dexType_),
            ":",
            LibString.toString(hookAddress_)
        ));


        DexState memory preDs_ = getDexState(dexPool_, dex_);
        vm.prank(address(admin_));
        FluidDexT1Admin(address(dex_)).updateHookAddress(
            hookAddress_
        );
        DexState memory postDs_ = getDexState(dexPool_, dex_);

        assertEq(hookAddress_, postDs_.d2.hookDeploymentNonce, string(abi.encodePacked(assertMessage_, "hookDeploymentNonce")));
    ...
    function _testUpdateUserSupplyConfig(
        ...
        assertEq(preState_.isUserAllowed, false, string(abi.encodePacked(assertMessage_, "DexUserSupplyData: !isUserAllowed")));
        ...
        assertEq(postState_.isUserAllowed, true, string(abi.encodePacked(assertMessage_, "DexUserSupplyData: isUserAllowed")));
        assertEq(preState_.userShares, postState_.userShares, string(abi.encodePacked(assertMessage_, "DexUserSupplyData: userShares")));
        assertEq(preState_.previousUserLimit, postState_.previousUserLimit, string(abi.encodePacked(assertMessage_, "previousUserLimit: shiftTime")));
        assertEq(preState_.lastTimestamp, postState_.lastTimestamp, string(abi.encodePacked(assertMessage_, "DexUserSupplyData: lastTimestamp")));
        assertEq(expandPercent_, postState_.expandPercent, string(abi.encodePacked(assertMessage_, "DexUserSupplyData: expandPercent")));
        assertEq(expandDuration_, postState_.expandDuration, string(abi.encodePacked(assertMessage_, "DexUserSupplyData: expandDuration")));
        assertEq(_convertNormalToBNToNormalLimit(withdrawLimit_), postState_.baseLimit, string(abi.encodePacked(assertMessage_, "DexUserSupplyData: baseLimit")));
    }
    ...
    function _testUpdateUserBorrowConfig(
        ...
        assertEq(preState_.isUserAllowed, false, string(abi.encodePacked(assertMessage_, "DexUserBorrowData: !isUserAllowed")));
        ...
        assertEq(postState_.isUserAllowed, true, string(abi.encodePacked(assertMessage_, "DexUserBorrowData: isUserAllowed")));
        assertEq(preState_.userShares, postState_.userShares, string(abi.encodePacked(assertMessage_, "DexUserBorrowData: userShares")));
        assertEq(preState_.previousUserLimit, postState_.previousUserLimit, string(abi.encodePacked(assertMessage_, "previousUserLimit: shiftTime")));
        assertEq(preState_.lastTimestamp, postState_.lastTimestamp, string(abi.encodePacked(assertMessage_, "DexUserBorrowData: lastTimestamp")));
        assertEq(expandPercent_, postState_.expandPercent, string(abi.encodePacked(assertMessage_, "DexUserBorrowData: expandPercent")));
        assertEq(expandDuration_, postState_.expandDuration, string(abi.encodePacked(assertMessage_, "DexUserBorrowData: expandDuration")));

        assertEq(_convertNormalToBNToNormalLimit(baseBorrowLimit_), postState_.baseLimit, string(abi.encodePacked(assertMessage_, "DexUserBorrowData: baseLimit")));
        assertEq(_convertNormalToBNToNormalLimit(maxBorrowLimit_), postState_.maxLimit, string(abi.encodePacked(assertMessage_, "DexUserBorrowData: maxLimit")));
    }
    ...
    function _testSwapExactInForthEnablePoolSwapBack(
        ...
        if (dexType_ == DexType.SmartDebt) {
            ...
            uint256 ethValue_ = _getSmartDebtOrColEthAmount(dexPool_, dex_, 1e4 * dexPool_.token0Wei, 0);
            vm.prank(alice);
            FluidDexT1Admin(address(dex_)).turnOnSmartDebt{value: ethValue_}(1e4 * dexPool_.token0Wei);
        }

        vm.recordLogs();
        uint256 amountInBack_ = _testSwapExactIn(dexPool_, user_, amountOut_, !startSwap0To1, dexType_, true, true, false);
        ...
        for(uint256 i = 0; i < entries.length; i++) {
            ...
            if (address(uint160(uint256(l_.topics[2]))) != (startSwap0To1 ? dexPool_.token1 : dexPool_.token0)) continue;
            if (dexType_ == DexType.SmartDebt) {
        ...
        assertGe(amountIn_ * 1001 / 1000, amountInBack_, string(abi.encodePacked(assertMessage_, "amountIn_ is less than amountInBack_")));
    ...
    function _testUpdateOracleWithTimes(
        ...
        for (uint256 i = 0; i < casesToRun_; i++) {
            ...
            {
                v_.changeInPriceBeforeSwap = int(ORACLE_PRECISION) - int((d_.lastToLastPrice * ORACLE_PRECISION) / d_.lastPrice);
                ...
                assertApproxEqAbs(d_.lastPrice, v_.dexVariablesAfterSwap.lastToLastPrice, 0, string(abi.encodePacked(assertMessage_, "lastToLastPrice")));
                d_.lastToLastPrice = d_.lastPrice;
                d_.lastPrice = v_.dexVariablesAfterSwap.lastPrice;

                (v_.oracleMapData, v_.oracleSlotConfig) = _getOracleMapData(dex_, v_.dexVariablesBeforeSwap.oracleMap, v_.dexVariablesBeforeSwap.oracleSlot);
                d_.totalSlots += v_.oracleSlotConfig.totalSlots;
                d_.totalCycle += v_.oracleSlotConfig.totalCycle;

                v_.changeInPriceAfterSwapWithMax5Percent = v_.oracleMapData.changePriceSign == 1 ? int256(v_.oracleMapData.changeInPriceWithMax5Percent) : -int256(v_.oracleMapData.changeInPriceWithMax5Percent);
                assertApproxEqAbs(v_.changeInPriceBeforeSwapWithMax5Percent, v_.changeInPriceAfterSwapWithMax5Percent, 0, string(abi.encodePacked(assertMessage_, "changeInPriceWithMax5Percent"))); 
                assertApproxEqAbs(v_.oracleMapData.timeDiff, v_.dexVariablesBeforeSwap.lastTimestampDifBetweenLastToLastPrice, 0, string(abi.encodePacked(assertMessage_, "timeDiff")));
            }
        ...
        {
            assertApproxEqAbs(d_.totalSlots, (_getDexVariablesData(dex_).oracleMap * 8) + (7 - _getDexVariablesData(dex_).oracleSlot), 0, string(abi.encodePacked(assertMessage_, "no of slots didn't match")));
            d_.overallOracleMapDataAtEnd = _getOracleAllSlotsData(dex_);
            assertApproxEqAbs(d_.overallOracleMapDataAtEnd.length, casesToRun_, 0, string(abi.encodePacked(assertMessage_, "no of cases didn't match")));

            for (uint i = 0; i < d_.overallOracleMapDataAtEnd.length; i++) {
                assertApproxEqAbs(d_.overallOracleMapDataAtEnd[i].changeInPriceWithMax5Percent, d_.overallOracleMapDataAfterEachSwap[i].changeInPriceWithMax5Percent, 0, string(abi.encodePacked(assertMessage_, "changeInPriceWithMax5Percent Overall")));
                assertApproxEqAbs(d_.overallOracleMapDataAtEnd[i].timeDiff, d_.overallOracleMapDataAfterEachSwap[i].timeDiff, 0, string(abi.encodePacked(assertMessage_, "timeDiff Overall")));
                assertApproxEqAbs(d_.overallOracleMapDataAtEnd[i].changePriceSign, d_.overallOracleMapDataAfterEachSwap[i].changePriceSign, 0, string(abi.encodePacked(assertMessage_, "changePriceSign Overall")));
            }
        }
    }
    ...
    function _testUpdateOracleOverLimit(
        ...
        {
            uint256 totalSlots_ =  
                (d_.totalCycle * dex_.constantsView().oracleMapping * 8) +
                (d_.dexVariables.oracleMap * 8) +
                (7 - d_.dexVariables.oracleSlot);
            assertApproxEqAbs(d_.totalSlots, totalSlots_, 0, string(abi.encodePacked(assertMessage_, "no of slots didn't match")));
            d_.overallOracleMapDataAtEnd = _getOracleAllSlotsData(dex_);
        }
    ...
    function _calculateRange(FluidDexT1 dex_) internal returns(RangeParams memory r_) {
        ...
        r_.upperThreshold = (r_.centerPrice + (r_.upperRange - r_.centerPrice) * (1e3 - dexVariables2_.upperShiftPercent / 1e3) / 1e3);
        r_.lowerThreshold = (r_.centerPrice - (r_.centerPrice - r_.lowerRange) * (1e3 - dexVariables2_.lowerShiftPercent / 1e3) / 1e3);
    }

    function _validateThresholdShift(FluidDexT1 dex_, uint256[] memory timeShiftInPercentage_, bool completeShift_, string memory assertMessage_) internal {
        ...
        v_.upperThreshold = (v_.centerPrice + (v_.upperRange - v_.centerPrice) * (1e3 - v_.dexVariables2.upperShiftPercent / 1e3) / 1e3);
        v_.lowerThreshold = (v_.centerPrice - (v_.centerPrice - v_.lowerRange) * (1e3 - v_.dexVariables2.lowerShiftPercent / 1e3) / 1e3);
        ...
        for (uint256 i = 0; i < v_.timeShifts.length; i++) {
            ...
            if (v_.dexVariables.lastPrice > v_.upperThreshold) {
                if (v_.totalShiftTime >= v_.shiftingTime) {
                    ...
                    changeCenterPrice_ = v_.centerPrice + ((v_.upperRange - v_.centerPrice) * v_.totalShiftTime / v_.shiftingTime);
                }
            } else {
                if (v_.totalShiftTime >= v_.shiftingTime) {
            ...
            _comparePrecision(postPex_.centerPrice, changeCenterPrice_, 1e18, 0, string(abi.encodePacked(assertMessage_, "_validateThresholdShift -", LibString.toString(v_.timeShifts[i]))));
            // TODO: validate range
        }
    ...
    function _validateTwap(
        ...
        for(uint256 j = 0; j < p_.twaps.length; j++) {
            ...
            if (j == 0) {
                ...
                p_.aggTwap1By0 = (
                    (p_.aggTwap1By0 * secondsAgo_[j - 1]) + 
                    (twap_.twap1by0 * (secondsAgo_[j] - secondsAgo_[j - 1]))
                ) / twapTime_;
                p_.aggTwap0By1 = (
                    (p_.aggTwap0By1 * secondsAgo_[j - 1]) + 
                    (twap_.twap0by1 * (secondsAgo_[j] - secondsAgo_[j - 1]))
                ) / twapTime_;
            }
            ...
            if (p_.totalTwapTimeCal >= twapTime_ && j == 0) {
                ...
                if (p_.totalTwapTimeCal + p_.dexVariables.lastTimestampDifBetweenLastToLastPrice >= twapTime_) {
                    ...
                    for (uint k = p_.oracleMapData.length - 1; k >= 0; k--) {
            ...
            {
                assertApproxEqAbs(twap1by0_, p_.aggTwap1By0, 1, string(abi.encodePacked(assertMessage_, "twap_.twap1by0")));
                assertApproxEqAbs(twap0by1_, p_.aggTwap0By1, 1, string(abi.encodePacked(assertMessage_, "twap_.twap0by1")));
            }
        }
    }
...
contract PoolT1Admin is PoolT1BaseTest {
    function testAdmin_UpdateRangePercents() public {
        ...
        DexParams[10] memory pools = [DAI_USDC, USDT_USDC, DAI_SUSDE, USDT_SUSDE, DAI_USDC_WITH_LESS_THAN_ONE, DAI_USDC_WITH_MORE_THAN_ONE, USDC_ETH, DAI_USDC_WITH_80_20, DAI_USDC_WITH_50_5, DAI_USDC_WITH_10_1];
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < shiftTimes_.length; k++) {
                    _testUpdateRangePercents(pools[i], admin, upperPercent_, lowerPercent_, shiftTimes_[k], poolTypes_[j]);
    ...
    function testAdmin_UpdateThresholdPercent() public {
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < shiftTimes_.length; k++) {
                    _testUpdateThresholdPercent(pools[i], admin, upperPercent_, lowerPercent_, 1 days, shiftTimes_[k], poolTypes_[j]);
                    vm.revertTo(snapshotId_);
                }
            }
        }
            ...
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < times_.length; k++) {
                    for(uint256 l = 0; l < centerPriceAddressNonces_.length; l++) {
                        _testUpdateCenterPriceAddress(pools[i], admin, centerPriceAddressNonces_[l], percent_, times_[k], poolTypes_[j]);
                        vm.revertTo(snapshotId_);
                    }
                }
            }
            ...
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 l = 0; l < 2; l++) {
                    ...
                    { // withdrawPerfectColLiquidity
                        ...
                        _testWithdrawPerfectColLiquidity(pools[i], eve, 40 * 1e18, poolTypes_[j], abi.encodeWithSelector(FluidDexErrors.FluidDexError.selector, FluidDexTypes.DexT1__WithdrawLimitReached));
                        vm.revertTo(snapshotId2_);
                    }

                    { // withdrawColLiquidity
                        (uint256 token0Amount_, uint256 token1Amount_) = _testWithdrawPerfectColLiquidity(pools[i], eve, 10 * 1e18, poolTypes_[j], new bytes(0));
                        
                        _testWithdrawColLiquidity(pools[i], eve, token0Amount_ * 5 / pools[i].token0Wei, token1Amount_ * 4 / pools[i].token1Wei, poolTypes_[j], abi.encodeWithSelector(FluidDexErrors.FluidDexError.selector, FluidDexTypes.DexT1__WithdrawLimitReached));
                        vm.revertTo(snapshotId2_);
                    }
                    ...
                    { // withdrawPerfectInOne
                        ...
                        _testWithdrawPerfectInOne(pools[i], eve, 40 * 1e18, true, poolTypes_[j], abi.encodeWithSelector(FluidDexErrors.FluidDexError.selector, FluidDexTypes.DexT1__WithdrawLimitReached));
                        vm.revertTo(snapshotId2_);
                    }
            ...
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 l = 0; l < 2; l++) {
                    ...
                    { // withdrawPerfectColLiquidity
                        ...
                        _testBorrowPerfectDebtLiquidity(pools[i], eve, 250 * 1e18, poolTypes_[j], abi.encodeWithSelector(FluidDexErrors.FluidDexError.selector, FluidDexTypes.DexT1__DebtLimitReached));
                        vm.revertTo(snapshotId2_);
                    }

                    { // withdrawColLiquidity
                        (uint256 token0Amount_, uint256 token1Amount_) = _testBorrowPerfectDebtLiquidity(pools[i], eve, 50 * 1e18, poolTypes_[j], new bytes(0));
                        
                        _testBorrowDebtLiquidity(pools[i], eve, token0Amount_ * 5 / pools[i].token0Wei, token1Amount_ * 4 / pools[i].token1Wei, poolTypes_[j], abi.encodeWithSelector(FluidDexErrors.FluidDexError.selector, FluidDexTypes.DexT1__DebtLimitReached));
                        vm.revertTo(snapshotId2_);
                    }
...
contract PoolT1Base is PoolT1BaseTest {
    ...
    function testDexT1_WithdrawPerfectColLiquidity() public {
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                ...
                (uint256 token0AmountDeposit_, uint256 token1AmountDeposit_) = _testDepositPerfectColLiquidity(dexPool_, alice, sharesAmount_, poolTypes_[j]);

                (uint256 token0AmountWithdraw_, uint256 token1AmountWithdraw_) = _testWithdrawPerfectColLiquidity(dexPool_, alice, sharesAmount_, poolTypes_[j], new bytes(0));

                assertGt(token0AmountDeposit_, token0AmountWithdraw_, string(abi.encodePacked(assertMessage_, "token0AmountDeposit_ < token0AmountWithdraw_")));
                _comparePrecision(
                ...
                assertGt(token1AmountDeposit_, token1AmountWithdraw_, string(abi.encodePacked(assertMessage_, "token1AmountDeposit_ < token1AmountWithdraw_")));
                _comparePrecision(
    ...
    function testDexT1_PaybackPerfectDebtLiquidity() public {
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                ...
                (uint256 token0AmountBorrow_, uint256 token1AmountBorrow_) = _testBorrowPerfectDebtLiquidity(pools[i], alice, sharesAmount_, poolTypes_[j], new bytes(0));

                (uint256 token0AmountPayback, uint256 token1AmountPayback) = _testPaybackPerfectDebtLiquidity(pools[i], alice, sharesAmount_, poolTypes_[j]);

                assertLt(token0AmountBorrow_, token0AmountPayback, string(abi.encodePacked(assertMessage_, "token0AmountBorrow_ < token0AmountPayback")));
                _comparePrecision(
                    token0AmountPayback,
                    token0AmountBorrow_,
                    pools[i].token0Wei,
                ...
                assertLt(token1AmountBorrow_, token1AmountPayback, string(abi.encodePacked(assertMessage_, "token1AmountBorrow_ < token1AmountPayback")));
                _comparePrecision(
                    token1AmountPayback,
                    token1AmountBorrow_,
                    pools[i].token1Wei,
    ...
    function testDexT1_DepositColLiquidity_() public {
        ...
        a_[0] = TokenAmountParams({
            token0Amount: 1e4,
            token1Amount: 0
        });

        a_[1] = TokenAmountParams({
            token0Amount: 0,
            token1Amount: 1e4
        });

        a_[2] = TokenAmountParams({
            token0Amount: 2 * 1e4,
            token1Amount: 1 * 1e4
        });

        a_[3] = TokenAmountParams({
            token0Amount: 1 * 1e4,
            token1Amount: 2 * 1e4
        });
    ...
    function testDexT1_DepositColLiquidityWithReserveZero() public {
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < a_.length; k++) {
                        FluidDexT1 dex_ = _getDexType(pools[i], poolTypes_[j], false);
                        StateData memory s_ = getState(pools[i], dex_, address(alice));

                        vm.prank(address(dex_));
                        FluidLiquidityUserModule(address(liquidity)).operate(
                            a_[k].token0Amount == 0 ? pools[i].token0 : pools[i].token1,
                            -int256(a_[k].token0Amount == 0 ? s_.liquidityToken0SupplyReserve : s_.liquidityToken1SupplyReserve),
                            0,
                            alice,
                            address(0),
                            new bytes(0)
    ...
    function testDexT1_DepositColLiquidityInTwoHalfs() public {
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                ...
                (uint256 token0Amount_, uint256 token1Amount_) = _testDepositPerfectColLiquidity(pools[i], alice, shareAmount_, poolTypes_[j]);
                ...
                assertGt(shareAmount_, share0_ + share1_, string(abi.encodePacked(assertMessage_, "shareAmount_ < share0_ + share1_")));
                _comparePrecision(
                    shareAmount_,
                    share0_ + share1_,
                    1e18,
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < a_.length; k++) {
                    ...
                    uint256 depositShares_ = _testDepositColLiquidity(pools[i], alice, a_[k].token0Amount, a_[k].token1Amount, poolTypes_[j]);

                    uint256 withdrawShares_ = _testWithdrawColLiquidity(pools[i], alice, a_[k].token0Amount, a_[k].token1Amount, poolTypes_[j], new bytes(0));

                    assertLe(depositShares_, withdrawShares_, string(abi.encodePacked(assertMessage_, "depositShares_ are more than withdrawShares_")));
                    ...
                    if (poolTypes_[j] == DexType.SmartColAndDebt) {
                        assertApproxEqAbs(depositShares_, withdrawShares_, type(uint64).max, string(abi.encodePacked(assertMessage_, "depositShares_ != withdrawShares_")));
                    } else {
                        _comparePrecision(depositShares_, withdrawShares_, 1e18, 200, string(abi.encodePacked(assertMessage_, "depositShares_ != withdrawShares_")));
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < a_.length; k++) {
                    ...
                    uint256 withdrawShares0_ = _testWithdrawColLiquidity(pools[i], alice, a_[k].token0Amount, 0, poolTypes_[j], new bytes(0));
                    uint256 withdrawShares1_ = _testWithdrawColLiquidity(pools[i], alice, 0, a_[k].token1Amount, poolTypes_[j], new bytes(0));

                    assertLe(depositShares_, withdrawShares0_ + withdrawShares1_, string(abi.encodePacked(assertMessage_, "depositShares_ are more than withdrawShares_")));

                    // TODO @thrilok: kept reduced precision to by 2 more decimals.
                    _comparePrecision(depositShares_, withdrawShares0_ + withdrawShares1_, 1e18 * 100, 0, string(abi.encodePacked(assertMessage_, "depositShares_ != withdrawShares_")));
    ...
    function testDexT1_BorrowDebtLiquidity() public {
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < a_.length; k++) {
                    _testBorrowDebtLiquidity(pools[i], alice, a_[k].token0Amount, a_[k].token1Amount, poolTypes_[j], new bytes(0));
                    vm.revertTo(snapshotId_);
                }
            }
        }
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < a_.length; k++) {
                    ...
                    uint256 borrowShares_ = _testBorrowDebtLiquidity(pools[i], alice, a_[k].token0Amount, a_[k].token1Amount, poolTypes_[j], new bytes(0));

                    uint256 paybackShares_ = _testPaybackDebtLiquidity(pools[i], alice, a_[k].token0Amount, a_[k].token1Amount, poolTypes_[j]);

                    assertGt(borrowShares_, paybackShares_, string(abi.encodePacked(assertMessage_, "borrowShares_ are more than paybackShares_")));
            ...
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < a_.length; k++) {
                    ...
                    ));
                    {
                        ...
                        (uint256 tokenDeposit0_, uint256 tokenDeposit1_) = _testDepositPerfectColLiquidity(pools[i], alice, a_[k].shares, poolTypes_[j]);
                    ...
                    uint256 depositShares0_ = _testDepositColLiquidityInWei(pools[i], alice, b_.txOne.token0Amount, b_.txOne.token1Amount, poolTypes_[j]);
                    uint256 depositShares1_ = _testDepositColLiquidityInWei(pools[i], alice, b_.txTwo.token0Amount, b_.txTwo.token1Amount, poolTypes_[j]);
                    (uint256 tokenWithdraw0_, uint256 tokenWithdraw1_) = _testWithdrawPerfectColLiquidity(pools[i], alice, (depositShares0_ + depositShares1_), poolTypes_[j], new bytes(0));

                    assertGt((b_.txOne.token0Amount + b_.txTwo.token0Amount), tokenWithdraw0_, "token0Amount < tokenWithdraw0_");
                    assertGt((b_.txOne.token1Amount + b_.txTwo.token1Amount), tokenWithdraw1_, "token1Amount < tokenWithdraw1_");
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < a_.length; k++) {
                    ...
                    assertGt(a_[k].shares, depositShares0_ + depositShares1_, string(abi.encodePacked(assertMessage_, "depositShares0_ + depositShares1_ > a_[k].shares")));
                    TokenAmountParamsDouble memory withdraw_;

                    (withdraw_.txTwo.token0Amount, withdraw_.txTwo.token1Amount) = _testWithdrawPerfectInOne(pools[i], alice, depositShares1_, b_.txTwo.token0Amount != 0, poolTypes_[j], new bytes(0));
                    if (b_.txTwo.token0Amount != 0) {
                        assertGt((b_.txTwo.token0Amount), withdraw_.txTwo.token0Amount, string(abi.encodePacked(assertMessage_, " txTwo token0Amount < withdraw_.txTwo.token0Amount")));
                        _comparePrecision(
                            ( b_.txTwo.token0Amount),
                            withdraw_.txTwo.token0Amount,
                            pools[i].token0Wei,
                            0,
                            string(abi.encodePacked(assertMessage_, "txTwo token0Amount != withdraw_.txTwo.token0Amount"))
                        );
                    } else {
                        assertGt((b_.txTwo.token1Amount), withdraw_.txTwo.token1Amount, string(abi.encodePacked(assertMessage_, " txTwo token1Amount < withdraw_.txTwo.token1Amount")));
                        _comparePrecision(
                            (b_.txTwo.token1Amount),
                            withdraw_.txTwo.token1Amount,
                            pools[i].token1Wei,
                            0,
                            string(abi.encodePacked(assertMessage_, "txTwo token1Amount != withdraw_.txTwo.token1Amount"))
                        );
                    }
                
                    (withdraw_.txOne.token0Amount, withdraw_.txOne.token1Amount) = _testWithdrawPerfectInOne(pools[i], alice, depositShares0_, b_.txOne.token0Amount != 0, poolTypes_[j], new bytes(0));
            ...
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < a_.length; k++) {
                    ...
                    ));
                    {
                        ...
                        (uint256 tokenBorrow0_, uint256 tokenBorrow1_) = _testBorrowPerfectDebtLiquidity(pools[i], alice, a_[k].shares, poolTypes_[j], new bytes(0));
                        b_ = TokenAmountParamsDouble({
                            txOne: TokenAmountParams({
                    ...
                    uint256 borrowShares0_ = _testBorrowDebtLiquidityInWei(pools[i], alice, b_.txOne.token0Amount, b_.txOne.token1Amount, poolTypes_[j], new bytes(0));
                    uint256 borrowShares1_ = _testBorrowDebtLiquidityInWei(pools[i], alice, b_.txTwo.token0Amount, b_.txTwo.token1Amount, poolTypes_[j], new bytes(0));
                    ...
                    (uint256 tokenPayback0_, uint256 tokenPayback1_) = _testPaybackPerfectDebtLiquidity(pools[i], alice, (borrowShares0_ + borrowShares1_), poolTypes_[j]);
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < 2; k++) {
                    ...
                    uint256 share_ = _testDepositColLiquidity(pools[i], alice, a_[k].token0Amount, a_[k].token1Amount, poolTypes_[j]);

                    (uint256 withdrawToken0_, uint256 withdrawToken1_) = _testWithdrawPerfectInOne(pools[i], alice, share_, a_[k].token0Amount != 0, poolTypes_[j], new bytes(0));

                    if (a_[k].token0Amount != 0) assertGt(a_[k].token0Amount * pools[i].token0Wei, withdrawToken0_, string(abi.encodePacked(assertMessage_, "token0Amount > withdrawToken0_")));
                    if (a_[k].token1Amount != 0) assertGt(a_[k].token1Amount * pools[i].token1Wei, withdrawToken1_, string(abi.encodePacked(assertMessage_, "token1Amount > withdrawToken1_")));

                    if (a_[k].token0Amount != 0) _comparePrecision(a_[k].token0Amount * pools[i].token0Wei, withdrawToken0_, pools[i].token0Wei, 0, string(abi.encodePacked(assertMessage_, "token0Amount != withdrawToken0_")));
                    if (a_[k].token1Amount != 0) _comparePrecision(a_[k].token1Amount * pools[i].token1Wei, withdrawToken1_, pools[i].token1Wei, 0, string(abi.encodePacked(assertMessage_, "token1Amount != withdrawToken1_")));
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < 2; k++) {
                    ...
                    (uint256 token0AmountBorrow_, uint256 token1AmountBorrow_) = _testBorrowPerfectDebtLiquidity(pools[i], alice, 1e3 * 1e18, poolTypes_[j], new bytes(0));

                    (uint256 token0AmountPayback_, uint256 token1AmountPayback_) = _testPaybackPerfectInOneToken(pools[i], alice, 1e3 * 1e18, k == 0, poolTypes_[j]);
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
                for(uint256 k = 0; k < a_.length; k++) {
                    ...
                    (uint256 shares_) = _testBorrowDebtLiquidity(pools[i], alice, a_[k].token0Amount, a_[k].token1Amount, poolTypes_[j], new bytes(0));

                    (uint256 token0AmountPayback_, uint256 token1AmountPayback_) = _testPaybackPerfectInOneToken(pools[i], alice, shares_, a_[k].token0Amount != 0, poolTypes_[j]);
...
contract PoolT1Oracle is PoolT1BaseTest {
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            for(uint256 j = 0; j < poolTypes_.length; j++) {
               string memory assertMessage_ = string(abi.encodePacked(
                ...
                for (uint k = 0; k < totalTime_ % 13; k++) {
                    uint256[] memory secondsAgo_ = new uint256[](k + 1);
                    for (uint l = 0; l < k + 1; l++) {
                ...
                for (uint k = 0; k < (totalTime_ % 13) - 3; k++) {
                    uint256[] memory secondsAgo_ = new uint256[](k + 1);
                    for (uint l = 0; l < k + 1; l++) {
                        secondsAgo_[l] = 13 * (l + 1);
                    }
    ...
    function testDexT1_UpdateOracleWithTimes() public {
        uint256[250] memory times_ = [uint256(1914706), uint256(2126517), uint256(1080588), uint256(3339778), uint256(3524166), uint256(746203), uint256(1094671), uint256(4182004), uint256(1594565), uint256(1937978), uint256(2696768), uint256(1543254), uint256(653003), uint256(3417180), uint256(2756766), uint256(3414311), uint256(4039049), uint256(1684430), uint256(4145736), uint256(1326451), uint256(2034953), uint256(51172), uint256(1268060), uint256(1743949), uint256(3459068), uint256(3124833), uint256(566309), uint256(3882476), uint256(3155612), uint256(2613733), uint256(1139690), uint256(2043388), uint256(229531), uint256(1068482), uint256(932262), uint256(2018110), uint256(2102803), uint256(3536180), uint256(2555217), uint256(2929084), uint256(3241633), uint256(3730809), uint256(822908), uint256(881910), uint256(3829849), uint256(4183406), uint256(788376), uint256(556816), uint256(3967281), uint256(3995677), uint256(461), uint256(416), uint256(319), uint256(44), uint256(73), uint256(264), uint256(462), uint256(221), uint256(322), uint256(89), uint256(498), uint256(183), uint256(481), uint256(368), uint256(400), uint256(476), uint256(114), uint256(126), uint256(76), uint256(347), uint256(131), uint256(103), uint256(256), uint256(296), uint256(484), uint256(368), uint256(52), uint256(137), uint256(94), uint256(153), uint256(463), uint256(401), uint256(176), uint256(138), uint256(261), uint256(219), uint256(45), uint256(496), uint256(194), uint256(121), uint256(9), uint256(487), uint256(182), uint256(4), uint256(82), uint256(92), uint256(185), uint256(309), uint256(475), uint256(498), uint256(336), uint256(495), uint256(3), uint256(137), uint256(441), uint256(143), uint256(395), uint256(426), uint256(182), uint256(122), uint256(501), uint256(450), uint256(227), uint256(83), uint256(70), uint256(25), uint256(143), uint256(223), uint256(251), uint256(85), uint256(199), uint256(92), uint256(262), uint256(402), uint256(439), uint256(505), uint256(89), uint256(257), uint256(440), uint256(510), uint256(161), uint256(167), uint256(402), uint256(46), uint256(411), uint256(305), uint256(399), uint256(411), uint256(141), uint256(273), uint256(174), uint256(190), uint256(470), uint256(2), uint256(264), uint256(176), uint256(298), uint256(473), uint256(101), uint256(115), uint256(382), uint256(431), uint256(65), uint256(243), uint256(9), uint256(221), uint256(428), uint256(350), uint256(180), uint256(228), uint256(194), uint256(156), uint256(97), uint256(493), uint256(9), uint256(111), uint256(315), uint256(287), uint256(249), uint256(188), uint256(136), uint256(296), uint256(91), uint256(269), uint256(91), uint256(2444281), uint256(1094285), uint256(2476384), uint256(2793588), uint256(3148257), uint256(1650301), uint256(2504633), uint256(145636), uint256(633371), uint256(2635847), uint256(1210751), uint256(1618747), uint256(915431), uint256(3171099), uint256(2934331), uint256(1540218), uint256(1832743), uint256(3125126), uint256(3627778), uint256(1611852), uint256(3906648), uint256(3425441), uint256(1313561), uint256(3261146), uint256(3113910), uint256(2445699), uint256(2000373), uint256(1197462), uint256(1421472), uint256(3456046), uint256(511), uint256(305), uint256(285), uint256(924971), uint256(216301), uint256(1466699), uint256(2624208), uint256(4055841), uint256(2650004), uint256(2856893), uint256(2031359), uint256(3658675), uint256(4076071), uint256(3962944), uint256(3669561), uint256(564985), uint256(577738), uint256(2473934), uint256(2126325), uint256(2696079), uint256(1936867), uint256(2537110), uint256(2632054), uint256(2448137), uint256(3544353), uint256(2124564), uint256(3741951), uint256(834251), uint256(4112510), uint256(3200780), uint256(1908994), uint256(2424569), uint256(268613), uint256(2481216), uint256(2130646), uint256(1243972), uint256(1683109), uint256(1138484), uint256(250377), uint256(3172629), uint256(3267467), uint256(3225130), uint256(1255809), uint256(2406667), uint256(946462)];
        DexParams[1] memory pools = [DAI_USDC];
        DexType[3] memory poolTypes_ = [DexType.SmartColAndDebt, DexType.SmartCol, DexType.SmartDebt];
...
contract PoolT1RangeTest is PoolT1BaseTest {
    function testDexT1_ThresholdShift() public {
        DexParams[4] memory pools = [DAI_USDC_WITH_LESS_THRESHOLD, DAI_USDC_WITH_80_20, DAI_USDC_WITH_50_5, DAI_USDC_WITH_10_1];
